...
const ANCHORS := {
    ...
    # ⚠️ 基于全身像构图的初值估算，需 gen_anchor_calibration.py 输出图肉眼校准微调
    #    （agent 当前无法读图，下列 cx/cy/w/h 为基于人物比例的最佳初值）。
    "res://assets/characters/messenger/messenger_portrait.png": {
        "tattoo":  {"cx": 0.62, "cy": 0.56, "w": 0.16, "h": 0.14},  # 手背锚形文身（画面右侧自然下垂的手，估算）
        "beard":   {"cx": 0.50, "cy": 0.24, "w": 0.20, "h": 0.16},  # 络腮胡（面部下方）
        "posture": {"cx": 0.50, "cy": 0.50, "w": 1.00, "h": 1.00},  # 笔挺站姿（全身）
        "manner":  {"cx": 0.50, "cy": 0.20, "w": 0.24, "h": 0.22},  # 神态（面部）
        "sleeve":  {"cx": 0.58, "cy": 0.60, "w": 0.16, "h": 0.16},  # 袖口磨损（右臂下端，估算）
        "limp":    {"cx": 0.52, "cy": 0.85, "w": 0.20, "h": 0.18},  # 轻微跛行（腿部，估算）
    },
