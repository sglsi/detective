# case_reasoning_registry.gd（案件推理链统一登记表）

> **源文件**：`res://data/case_reasoning_registry.gd`
> **性质**：跨场景推理链的整合层（节点真相源）。本 `.md` 是该 GDScript 文件的**独立副本**，供在 Typedown 中查看/编辑参考。
> ⚠️ 在 Typedown 中编辑本文件**不会**自动回写原 `.gd`；要修改实际逻辑请改 `case_reasoning_registry.gd`，或让我把改动同步回去。
...
- **本 .md 是副本**：Godot 运行的是 `case_reasoning_registry.gd`，不是这个 md。改 md 不影响游戏。
- **真想改推理链内容**：编辑各 `scripts/scene/scene2.gd ~ scene8.gd` 里的 `func reasoning_hypothesis()`（返回 `battlefield: {hypotheses, conclusions}`）。注册表只是把它们并集起来，不持有数据。
- **改本文件逻辑**（如 `SCENE_SCRIPTS` 列表、`get_hypo_union_up_to` 过滤规则）：请直接改 `.gd`，或告诉我，我来把 md 的改动同步回 `.gd`。
