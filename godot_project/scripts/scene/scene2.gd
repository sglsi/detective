...
#      → 花园通道勘查(脚印 c205-206 / 鞋印尺寸 c209-210 / 往返行踪 c211, sc02_path) → 推理。
...
const STREET_HOTSPOTS = [
	...
	{"id":"c202","attribute_tags":["直接物证"],"label":"平行车轮印","x":885,"y":649,"w":150,"h":42,
	 "desc":"用卷尺测量：前后两组车轮印的轴距约6英尺10英寸（82英寸），轮宽约2英寸，压痕最深处约1.2英寸。查知识库·伦敦马车类型：出租四轮马车（brougham型）轴距约6英尺10英寸，私家大型马车约8英尺6英寸、公共马车更长——轴距越短转弯半径越小，才能在伦敦小巷里穿行调头，这是一辆出租马车。","tool":"卷尺",
	...
	{"id":"c203","attribute_tags":["直接物证"],"label":"右前蹄新蹄铁","x":1000,"y":821,"w":150,"h":42,
	 ...
	 "image":"res://assets/scenes/sc02_street.png","anchor":"c203","relation_tags":["H2-02"]},
	{"id":"c204","attribute_tags":["直接物证"],"label":"马蹄印迹零乱","x":770,"y":789,"w":150,"h":42,
	 ...
	 "image":"res://assets/scenes/sc02_street.png","anchor":"c204","relation_tags":["H2-03","C2-02"]},
	# ── Q3 困难观察级干扰（巡逻警马蹄印）：correct=false，简单模式过滤剔除、普通30%、困难70%概率出现 ──
	{"id":"c207","attribute_tags":["直接物证"],"label":"另一组蹄印","x":560,"y":470,"w":150,"h":42,
	 "desc":"泥地里另有一组蹄印——蹄铁磨损均匀，大小也与那匹马不同。这是巡逻警马的蹄印：警方昨夜到场后留下的，并非来客留下的痕迹。别把它和那匹拉车的马蹄印混为一谈。","tool":"none",
...
const PATH_HOTSPOTS = [
	{"id":"c205","attribute_tags":["直接物证"],"label":"两组不同脚印","x":731,"y":584,"w":150,"h":42,
	 "desc":"泥地里有两组明显不同的脚印，彼此挨得很近、并肩而行，部分叠在另一部分上面（有先后顺序），且都在警察脚印之下——案发当晚有两个人来过。放大镜下看鞋形：一组鞋头方钝、个头大（方头靴）；一组鞋底窄尖、体态轻（尖头漆皮靴）。方头靴多为干体力活的人穿，漆皮靴多为体面人士。","tool":"卷尺",
	 "image":"res://assets/scenes/sc02_path.png","anchor":"c205","relation_tags":["H2-04","H2-06","H2-08","C2-01"]},
	{"id":"c206","attribute_tags":["直接物证"],"label":"步伐距离差异","x":923,"y":735,"w":150,"h":42,
	 "desc":"卷尺测量步长（一左一右两个脚印之间的距离）：方头靴约2.6~2.9英尺，尖头漆皮靴约2.4~2.7英尺。查知识库·步长与身高（步长约为身高的0.45倍，贝蒂荣）：2.75英尺→身高约6英尺（183cm）；2.55英尺→身高约5英尺8英寸（172cm）。两串脚印的步长差出一档——一个更高大，一个稍矮些，但都是成年男子。","tool":"卷尺",
	 "image":"res://assets/scenes/sc02_path.png","anchor":"c206","relation_tags":["H2-04","H2-05"]},
	# ── 新增：鞋印本身的尺寸，以及「往返」行踪（承接勘查对象C的六步闭环）──
	{"id":"c209","attribute_tags":["直接物证"],"label":"方头靴印","x":960,"y":250,"w":150,"h":42,
	 "desc":"卷尺与放大镜：方头靴印长11.5英寸、宽4英寸，印痕深约0.8英寸——是现场踩得最深的鞋印。鞋头方钝，后跟外侧磨损明显（久站或负重者的典型磨损图谱）。查知识库·足迹分析：鞋印长11.5英寸对应脚长约10.9英寸（27.8厘米），加上鞋头余量正是成年男子的靴底；印宽4英寸、压痕最深，说明来人体格健壮、体重不轻。","tool":"卷尺",
	 "image":"res://assets/scenes/sc02_path.png","anchor":"c209","relation_tags":["H2-06","H2-07"]},
	{"id":"c210","attribute_tags":["直接物证"],"label":"尖头漆皮靴印","x":1200,"y":380,"w":150,"h":42,
	 "desc":"卷尺与放大镜：尖头漆皮靴印长11英寸、宽3.5英寸，印痕深仅约0.5英寸——明显比那双方头靴浅。鞋型窄长、鞋头尖细、后跟细窄，漆皮表面光洁，是体面人士的正装靴。查知识库·足迹分析与步长身高：鞋印长11英寸对应脚长约10.4英寸（26.4厘米），鞋印窄长、落脚又浅，说明此人是个体态清瘦的中等身材成年男子，身高约5英尺8英寸（172厘米）。","tool":"卷尺",
	 "image":"res://assets/scenes/sc02_path.png","anchor":"c210","relation_tags":["H2-09"]},
	{"id":"c211","attribute_tags":["直接物证"],"label":"两串脚印的往返","x":960,"y":480,"w":150,"h":42,
	 "desc":"沿两串脚印一路追踪过去：方头靴那串是双趟——从花园小径进去，又从门口折返出来，有进有出；尖头漆皮靴那串只有单趟——进去了，却找不到出来的脚印。两个人是一起进去的，出来的只有一个。没出来的那个，多半还留在那栋房子里。","tool":"放大镜",
	 "image":"res://assets/scenes/sc02_path.png","anchor":"c211","relation_tags":["H2-10"]},
	# ── Q3 困难观察级干扰（巡警脚印）：correct=false，简单剔除、普通30%、困难70%概率出现 ──
	{"id":"c208","attribute_tags":["直接物证"],"label":"规整的靴印","x":540,"y":470,"w":150,"h":42,
	 "desc":"通道上还有一组规整的靴印，鞋头圆钝、步幅均匀——这是巡警来回勘查时留下的，案发之后才踩上去的。它与两位来客的大步方头靴、小步漆皮靴都不是一回事。","tool":"none",
...
func _npc_talk_text(gc: int) -> String:
	match gc:
		...
		2,3: return "福尔摩斯：\"用卷尺量量前后车轮印之间的纵向距离——那叫轴距（左右两轮的横向间距叫轮距，两者别混淆）。想知道是什么马车？去知识库查查伦敦马车的类型。\""
		4: return "福尔摩斯：\"看看地上——马的蹄印。用放大镜仔细看每个蹄印，蹄铁的磨损程度一样吗？再看蹄印的走向，是直直的还是乱的？\""
		5,6: return "福尔摩斯：\"看看这些脚印——仔细数，有几种不同的？用卷尺量量鞋印的长宽，再量步长（一左一右两个印之间的距离）——一个人的步长和他的身高有关系。\""
		7,8: return "福尔摩斯：\"沿着这两串脚印一路看过去，注意方向——哪一串进去又出来了，哪一串进去了却没再出来？\""
...
# ===== 双阶段观察器：街道勘查(c201-204) → 花园通道勘查(c205-206, c209-211) =====
...
func _observe_hint(_noun: String, _person: bool = false) -> String:
	...
	return "🔍 花园通道勘查 — " + ("所有可观察点已高亮，点击通道上的脚印与行踪圆圈" if (DifficultyManager and DifficultyManager.auto_reveal_clues) else "点击通道上的脚印尺寸与行踪标记点")
...
func _street_to_path_transition() -> void:
	...
	_current_observer().show()
	_ui.set_dialogue("提示", "街道痕迹已记录。雨后的花园通道还藏着脚步的秘密——注意两串脚印的长度、步长与行踪。")

# ===== 全部线索收集完成 =====
func _on_observe_complete() -> void:
	_ui.set_dialogue("华生", "福尔摩斯，花园里的痕迹全都记录好了——轴距6英尺10英寸的车轮印、右前蹄的新蹄铁、两串并肩的脚印（方头靴11.5英寸、尖头漆皮靴11英寸），还有一进一出与只进不出的行踪……证据很充足。", "思考")
...
func reasoning_hypothesis() -> Dictionary:
	return {
		...
		"description": "一辆伦敦出租马车停在花园街3号门口；泥地里有两组脚印——一高一矮，高个穿方头靴，两人并肩而行。\n\n推理层级（台词库§18）：场景二只作观察事实与浅层推论，「马车夫是凶手」为假设级结论，不作定论。\n\n活跃假设：\n· H2-01 来车是出租四轮马车（轴距6英尺10英寸）\n· H2-02 右前蹄铁新换——马被特别关照\n· H2-03 马曾无人看管——赶车人不在车上\n· H2-04 案发当晚有两个人来过（一高一矮）\n· H2-05 其中一人身高约6英尺（范围估计）\n· H2-06 高个子穿方头靴\n· H2-07 高个子体格强壮（弱-中·可选）\n· H2-08 两人并肩同行抵达（相距很近）\n· H2-09 尖头漆皮靴者体态清瘦（身高约5英尺8英寸）\n· H2-10 方头靴者离开、尖头靴者留在屋内\n\n矛盾标记：\n· C2-01 两组不同脚印（方头靴 vs 尖头漆皮靴）\n· C2-02 乘马车来 vs 泥地大步走\n· C2-03 新蹄铁一只 vs 三只旧蹄铁\n· C2-04 方头靴印双趟（有进有出）vs 尖头靴印单趟（只进无出）",
		"battlefield": {
		"hypotheses": [
			{"id":"H2-01","text":"来车是一辆出租四轮马车","kind":"true","correct":true,"dir":"affirm","subject":["出租马车","马车"],"object":["轴距","6英尺10英寸","82英寸","6.8英尺"],
			 "gate_clue_ids":["c201","c202"],
				 "adopt_desc":"轴距6英尺10英寸（82英寸）符合留存实车实测的出租四轮马车（brougham型）典型轴距；私家大型马车约8英尺6英寸、公共马车更长。轴距越短转弯半径越小，出租马车才能在伦敦小巷里穿行调头——私家大型马车不会这么短。",
				 "new_clue_hint":"知识库·伦敦马车类型：出租四轮马车（brougham型）轴距约6英尺10英寸；私家大型马车约8英尺6英寸。"},
			{"id":"H2-02","text":"右前蹄铁是新换的——这匹马最近被特别关照过","kind":"true","correct":true,"dir":"affirm","subject":["马","蹄铁"],"object":["右前蹄","新换"],
			 "gate_clue_ids":["c203"],
				 "adopt_desc":"右前蹄铁无磨损、边缘锐利，与其余三只磨损状态差异显著；正常使用的马四个蹄铁磨损应大致均匀。",
				 "new_clue_hint":"知识库·马蹄铁与马车：单个蹄铁特别新，说明该蹄铁最近更换过。"},
			{"id":"H2-03","text":"马曾无人看管——赶车的人不在车上","kind":"true","correct":true,"dir":"affirm","subject":["马","赶车人"],"object":["蹄印零乱","无人看管"],
			 "gate_clue_ids":["c204"],
				 "adopt_desc":"蹄印方向散乱、有迂回和停顿痕迹，非正常行进路线。如果有人驾驭，马不会走得这么乱——赶车的不在车上。",
				 "new_clue_hint":"知识库·马蹄铁与马车：马蹄印迹分布零乱（非直线行走），说明马曾无人驾驭。"},
			{"id":"H2-04","text":"案发当晚有两个人来过（一高一矮）","kind":"true","correct":true,"dir":"affirm","subject":["来客"],"object":["两个人","步长"],
			 "gate_clue_ids":["c205","c206"],
				 "adopt_desc":"泥地里有两组明显不同的脚印，彼此挨得很近、部分叠压且都在警察脚印之下——夜间来客共两人，都比警察的脚印早。方头靴那串长11.5英寸、步长约2.6~2.9英尺；尖头漆皮靴那串长11英寸、步长约2.4~2.7英尺。",
				 "new_clue_hint":""},
			{"id":"H2-05","text":"其中一人身高约6英尺（范围估计）","kind":"true","correct":true,"dir":"affirm","subject":["高个子"],"object":["身高","6英尺"],
			 "gate_clue_ids":["c206"],
				 "adopt_desc":"方头靴那串步长2.6~2.9英尺（中值2.75），按步长/身高比0.45推算，身高约6英尺（183cm）；鞋印长11.5英寸、宽4英寸、压痕最深，也符合成年男子的靴底。",
				 "new_clue_hint":"知识库·步长与身高：成人步长约为身高的0.45倍（贝蒂荣）；步幅（同脚相邻两印）约为步长的两倍，两者别混用。"},
			{"id":"H2-06","text":"高个子穿方头靴（体力活的穿法）","kind":"true","correct":true,"dir":"affirm","subject":["高个子"],"object":["方头靴","靴"],
			 "gate_clue_ids":["c205"],
				 "adopt_desc":"大步脚印的鞋头呈方形，非尖头皮靴，与方头靴特征吻合——穿这种靴子的多半是干体力活的。",
				 "new_clue_hint":"放大镜观察脚印形状：大步是方头靴，小步是漆皮靴。"},
			{"id":"H2-07","text":"高个子体格强壮","kind":"true","correct":true,"pool":"manual","dir":"affirm","subject":["高个子"],"object":["体格强壮","强壮"],
			 "gate_clue_ids":["c206"],
				 "adopt_desc":"步幅跨度与印痕深度显示来者体格强壮，属可选强化推断。",
				 "new_clue_hint":""},
			{"id":"H2-08","text":"两个来客是并肩同行抵达的（相距很近）","kind":"true","correct":true,"dir":"affirm","subject":["来客"],"object":["并肩","同行"],
			 "gate_clue_ids":["c205"],
				 "adopt_desc":"方头靴与尖头漆皮靴两串脚印始终挨得很近、并排向前，叠压有先后但方向一致——两人是同时到场、并肩走进去的，而非一前一后各自前来。",
				 "new_clue_hint":""},
			{"id":"H2-09","text":"穿尖头漆皮靴的是个体态清瘦的人（身高约5英尺8英寸）","kind":"true","correct":true,"dir":"affirm","subject":["另一来客"],"object":["身高","5英尺8英寸","清瘦"],
			 "gate_clue_ids":["c210","c206"],
				 "adopt_desc":"尖头漆皮靴那串步长2.4~2.7英尺（中值2.55）→身高约5英尺8英寸（172cm）；鞋印长11英寸、宽3.5英寸、印深约0.5英寸，明显比方头靴窄而浅——鞋型窄长、落脚轻，是个体态清瘦的体面人士，比高个子矮半头。",
				 "new_clue_hint":"知识库·步长与身高：鞋印窄长、压痕浅都与体态清瘦相符；脚长≈身高×0.15，鞋印长还要加上鞋头余量。"},
			{"id":"H2-10","text":"方头靴者离开、尖头靴者留在了屋内","kind":"true","correct":true,"dir":"affirm","subject":["来客"],"object":["双趟","单趟","留在屋内"],
			 "gate_clue_ids":["c211"],
				 "adopt_desc":"追踪两串脚印：方头靴那串是双趟（进去又折返出来，有进有出），尖头漆皮靴那串只有单趟（只进无出）。两人一同进去，出来的只有一个——没出来的那个仍在屋内。此为假设级结论，待室内证据验证。",
				 "new_clue_hint":"知识库·足迹分析：循连续足迹追踪可判断「来去路线、是否折返、是否携物」。"},
			{"id":"H2-M1","text":"来的是身材矮小的报童","kind":"mislead","correct":false,
			 "gate_clue_ids":["c205"],
			 "reject_desc":"报童小步与方头靴大码脚印矛盾：现场主要脚印是方头靴，尺寸与报童不合。",
			 "adopt_desc":""},
			{"id":"H2-M2","text":"来客徒步踏泥大步进入花园","kind":"mislead","correct":false,
			 "gate_clue_ids":["c202"],
			 "reject_desc":"若徒步进入，泥地应留有连续大步足迹；已见并行车轮印与碾轧花草，与乘马车抵达矛盾。",
			 "adopt_desc":""}
		],
		"contradictions": [
			{"id":"C2-01","text":"现场出现两组不同脚印（方头靴 vs 尖头漆皮靴）","correct":true},
			{"id":"C2-02","text":"乘马车来 vs 泥地大步走进入花园","correct":true},
			{"id":"C2-03","text":"一只马蹄铁是新的、其余三只是旧的","correct":true},
			{"id":"C2-04","text":"方头靴印双趟（有进有出）vs 尖头靴印单趟（只进无出）","correct":true}
		],
		"conclusions": [
			{"id":"CL2-1","text":"来的是一辆出租马车","kind":"true","dir":"affirm","subject":["出租马车"],"object":["轴距","6英尺10英寸","82英寸","6.8英尺"],"gate_hypo_ids":["H2-01"],
			 "adopt_desc":"三链之一：轴距6英尺10英寸符合留存实车实测的伦敦出租四轮马车（brougham型）——私家大型马车约8英尺6英寸，不会这么短。"},
			{"id":"CL2-2","text":"这匹马最近被特别关照过（右前蹄铁新换）","kind":"true","dir":"affirm","subject":["马"],"object":["右前蹄铁","新换"],"gate_hypo_ids":["H2-02"],
			 "adopt_desc":"三链之二：右前蹄铁是新的，其余三只磨损——这匹马最近被人特别关照过。"},
			{"id":"CL2-3","text":"赶车的人不在车上——他下了车","kind":"true","dir":"affirm","subject":["赶车人"],"object":["蹄印零乱","下车"],"gate_hypo_ids":["H2-03"],
			 "adopt_desc":"三链之三：马蹄印零乱无章，不是直线行进——赶车的人不在车上，他下了车。"},
			{"id":"CL2-4","text":"马车夫进了那栋空房子（三线合一·假设级）","kind":"true","dir":"affirm","subject":["马车夫"],"object":["空房子"],"gate_hypo_ids":["H2-01","H2-02","H2-03"],"target":"person:KILLER",
			 "adopt_desc":"一辆出租马车、一匹刚换过蹄铁的马、一个不在车上的车夫——他进了那栋空房子。此为假设级结论，待室内证据验证。"},
			{"id":"CL2-5","text":"夜间来客共两人，一高一矮","kind":"true","dir":"affirm","subject":["来客"],"object":["两个人","一高一矮"],"gate_hypo_ids":["H2-04"],
				 "adopt_desc":"两串脚印挨得极近、叠压在警察脚印之下——两个来客，一个步长2.6~2.9英尺，一个2.4~2.7英尺。"},
			{"id":"CL2-6","text":"高个子身高约6英尺、穿方头靴","kind":"true","dir":"affirm","subject":["高个子"],"object":["6英尺","方头靴"],"gate_hypo_ids":["H2-05","H2-06"],"target":"person:KILLER",
				 "adopt_desc":"步长2.6~2.9英尺推算身高约6英尺，鞋印长11.5英寸、宽4英寸、压痕最深；方头靴是体力活的穿法——高个子体貌的关键观察结论（范围估计，不作定论）。"},
			{"id":"CL2-7","text":"两个来客并肩同行、同乘一辆车抵达","kind":"true","dir":"affirm","subject":["来客"],"object":["并肩","同乘"],
			 "gate_hypo_ids":["H2-04","H2-08"],
				 "adopt_desc":"两组脚印挨得极近、并排向前——两人同时到场；结合门口那串出租马车的车辙，他们是同乘那一辆车来的。"},
			{"id":"CL2-8","text":"另一位来客是身高约5英尺8英寸的清瘦个子","kind":"true","dir":"affirm","subject":["另一来客"],"object":["5英尺8英寸","清瘦"],
			 "gate_hypo_ids":["H2-09"],
				 "adopt_desc":"尖头漆皮靴那串步长2.4~2.7英尺、印长11英寸、印深0.5英寸——身高约5英尺8英寸，体态清瘦、穿着体面。"},
			{"id":"CL2-9","text":"两人同入、一人独出——没出来的那个仍在屋内","kind":"true","dir":"affirm","subject":["来客"],"object":["双趟","单趟","屋内"],
			 "gate_hypo_ids":["H2-10"],
				 "adopt_desc":"方头靴双趟有进有出、尖头靴单趟只进无出：走出来的只有一个。留在屋里的那个，极可能就是室内那具尸体——此为假设级结论，待室内证据验证。"},
			{"id":"CL2-M1","text":"来客是身材矮小的少年","kind":"mislead","dir":"affirm","subject":["来客"],"object":["矮小","少年"],"gate_hypo_ids":["H2-M1"],
			 "reject_desc":"小步漆皮靴与方头靴主组矛盾：来客并非矮小少年。"},
			{"id":"CL2-1M","text":"来客徒步踏泥大步进入花园","kind":"mislead","dir":"affirm","subject":["来客"],"object":["徒步","泥地","大步"],"gate_hypo_ids":["H2-M2"],
			 "reject_desc":"并行车轮印与碾轧花草说明来客是乘马车抵达，非徒步大步进入。"},
			{"id":"CL2-2M","text":"来客身材矮小、体格瘦弱","kind":"mislead","dir":"affirm","subject":["来客"],"object":["矮小","瘦弱"],"gate_hypo_ids":["H2-05"],
				 "reject_desc":"现场方头靴步长（2.6~2.9英尺→身高6英尺）、印宽4英寸、压痕最深，均指向高大强壮者，与矮小瘦弱矛盾。"},
			{"id":"CL2-3M","text":"高个子穿小步漆皮靴","kind":"mislead","dir":"affirm","subject":["高个子"],"object":["漆皮靴","小步"],"gate_hypo_ids":["H2-06"],
			 "reject_desc":"大步脚印为方头靴，小步漆皮靴属另一位来客，两者不是同一人。"}
		],
		"milestones": [
			{"id":"S2-1","text":"死亡地点：劳瑞斯顿花园街3号"},
			{"id":"S2-2","text":"来的是一辆出租马车（轴距6英尺10英寸）"},
			{"id":"S2-3","text":"右前蹄铁新换——马被特别关照过"},
			{"id":"S2-4","text":"蹄印零乱——赶车人不在车上，下了车"},
			{"id":"S2-5","text":"三线合一：马车夫进了那栋空房子（假设级）"},
			{"id":"S2-6","text":"夜间来客两人：一高一矮，高个穿方头靴"},
			{"id":"S2-7","text":"方头靴印11.5×4英寸、步长2.6~2.9英尺：身高约6英尺的壮汉"},
			{"id":"S2-8","text":"尖头靴印单趟只进无出——他留在了屋里"},
		],
	},
	"chain_id": scene_id(),
...
func options_lines() -> Array:
	return [
		...
		"📏 卷尺：场景二起可用，测量轴距/鞋印尺寸/步长等物理证据",
		"💡 本案马车轴距≈6英尺10英寸（出租四轮马车 brougham 型；私家大型马车约8英尺6英寸）",
		"音效：MVP 阶段暂无（M3 补全）",
	]
...
func _enter_transition() -> void:
	...
	if diff == DifficultyManager.Difficulty.EASY:
		nodes = _make_nodes([
			...
			["cb","福尔摩斯","第一——车轮印的轴距6英尺10英寸，符合伦敦出租四轮马车（brougham型）。私家大型马车要8英尺6英寸，不会这么短，因为它们不钻小巷。→ 来的是一辆出租马车。","cc","指导"],
...
func _apply_restored_phase(p: int, ids: Array, _clues_arr: Array) -> bool:
	...
	match p:
		...
		Phase.TRANSITION:
			# 终局（已过场）读档：重放「阶段末过场对话」后展示「侦破过程」结束面板，
			# 而非直接跳到评价页。否则在过场对话中存档（验证后推进 1-2 步）读档会跳过该对话——用户报的跳流程 bug。
			# 重放后对话结束自动 _go_to_next_scene → _show_scene_rating（同正常流程）。
			_enter_transition()
			return true
	return false
...
func _show_scene2_verdict_dialogue(v: int, advance: bool) -> void:
	...
	match v:
		3:  # VERIFIED —— 总结式四档，对齐「三条线指向同一个人」
			...
			if diff == DifficultyManager.Difficulty.EASY:
				nodes.append(_mk_node("v2","福尔摩斯","第一——车轮印轴距6英尺10英寸，符合伦敦出租四轮马车（私家大型马车要8英尺6英寸）；第二——右前蹄铁是新的，这匹马最近被特别关照过；第三——马蹄印零乱，赶车的人不在车上，他下了车。","click",["v3"],[],"指导"))
