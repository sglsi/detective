...
const HOTSPOTS = [
	...
	# ── 勘查对象C：步伐距离（08稿 六步闭环）──
	{"id":"c205","label":"两组不同脚印","x":260,"y":630,"w":150,"h":42,
...
func _show_detective_dialogue() -> void:
	...
	_start_dialogue(_make_nodes([
		["b0","葛莱森","福尔摩斯先生，您总算来了。昨晚巡警在这座空屋里发现一具男尸——没有外伤，屋里的东西我们都没动。"],
		["b1","雷斯垂德","（拍了拍身上的灰）我也刚到。附近居民昨晚听到过马车声和马蹄声，天亮前还有人看见一个醉汉在街上踉跄。"],
		["b2","葛莱森","（压低声音）我看这是桩政治阴谋——某个秘密团体的灭口行动。雷斯垂德那套情杀论？太俗套。"],
		["b3","雷斯垂德","（哼了一声）我倒觉得是私人恩怨。死者衣着体面，因爱生恨才是最常见的杀人动机。某人总喜欢把事情想复杂。"],
		["b4","福尔摩斯","（并不接话，蹲下身看向泥地）两位的推论，都会进推理战场当'待验证假设'——但真相得靠证据说话。","","从容"],
		["b5","福尔摩斯","华生，先别进屋。雨后的泥地是最好的记录者：注意车轮印的间距、四个蹄印的磨损差异、还有脚印的多少和大小。你自己查，别被带偏。","","指导"]]), "b0", _on_detective_ended)
