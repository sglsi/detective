...
const HOTSPOTS = [
	...
	# 2026-08-15：热点坐标按 sc_02_garden.png 的实际内容重新分布：
	#   c201 碾轧的花草 → 左侧花园草地；
	#   c202/c204 放在左车道（车轮印/马蹄零乱）；
	#   c203/c206 放在右车道（新蹄铁/步伐差异）；
	#   c205 放在前景泥地中央（两组脚印）。
	# 每个热点都带 image+anchor，点击后像场景1一样弹出背景放大框+底部线索说明。
	{"id":"c201","label":"碾轧的花草","x":180,"y":860,"w":150,"h":42,
	...
	{"id":"c204","label":"马蹄印迹零乱","x":760,"y":820,"w":150,"h":42,
	...
	# ── 勘查对象C：步伐距离（08稿 六步闭环）──
	{"id":"c205","label":"两组不同脚印","x":1000,"y":880,"w":150,"h":42,
	...
	{"id":"c206","label":"步伐距离差异","x":1280,"y":850,"w":150,"h":42,
