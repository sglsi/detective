...
func _restore_saved_state() -> bool:
	...
	# 杜绝「场景内进度（_garden_obs 已记录数）」与「推理墙（ClueSystem 全局累计）」不一致的「两层皮」。
	_garden_clues = []
	if ClueSystem: ClueSystem.clear_source("garden")
	for cid in saved_ids:
		var h = _get_hotspot(cid)
		if not h.is_empty():
			_garden_clues.append(h)
			if ClueSystem: ClueSystem.collect_clue(cid, h.get("label",""), h.get("desc",""), h.get("correct", true), "garden")
	match saved_phase:
		Phase.ARRIVAL:
...
func _open_wall() -> void:
	...
	# 观察阶段手动开墙仅预览，不推进。注意：不能无条件置 false——
	# _enter_reasoning 先置 true 再调本函数，曾被这里覆盖导致验证后卡死。
...
func _do_save() -> void:
	...
	var ids: Array = []
	for c in _garden_clues: ids.append(c.get("id", ""))
	if ids.is_empty():
...
func _make_nodes(raw: Array) -> Array:
	...
	for i in raw.size():
		...
		# 避免原逻辑把末节点 next 指到不存在的虚拟节点（如 c3）而刷 ERROR。
		if i < last_idx:
			if len(r) > 3:
				nxt.append(r[3])
			else:
