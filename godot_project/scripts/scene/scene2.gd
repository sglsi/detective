...
func _enter_arrival() -> void:
	_start_dialogue(_make_nodes([
		...
		["a3","福尔摩斯","先别急着下结论，华生。看——栅栏外面还有一个人。雷斯垂德，你也来了？"]]), "a0", _on_arrival_ended)
...
func _show_detective_dialogue() -> void:
	...
	_start_dialogue(_make_nodes([
		...
		["b5","福尔摩斯","很好。华生，你戴上放大镜仔细检查地面——注意车轮印的间距、马蹄印中哪只新旧不一，以及脚印的多少和大小。我和警长们先在边上等着。"]]), "b0", _on_detective_ended)
...
func _enter_transition() -> void:
	...
	_start_dialogue(_make_nodes([
		...
		["c2","葛莱森","跟我来。死者的遗体还在里面没有动过。"]]), "c0", _go_to_next_scene)
