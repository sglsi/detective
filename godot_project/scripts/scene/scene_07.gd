extends GameScene
class_name Scene07Game

## 场景07：郝黎代旅馆 — 第二被害人 —— 自主探索（无引导）
...
func _setup_ui() -> void:
	...
	if scene_title:
		scene_title.set("text", "郝黎代旅馆 — 第二被害人")
	_setup_side_panel()
...
func _start_tutorial() -> void:
	scene_controller.load_scene("sc_07_hotel")
	await get_tree().create_timer(0.6).timeout
	dialogue_renderer.load_dialogue_resource("res://resources/dialogues/scene_07_hotel.tres")
...
func _clue_name(id: String) -> String:
	var names = "door_blood": "门缝血迹（斯特兰森被刺）", "pill_box": "药丸木匣（毒/无毒）", "purse": "钱袋（非谋财）", "wire": "电报 J.H.（杰弗森·霍普）"
	...
	match result:
		...
		ReasoningWallUI.VerifyResult.SUPPORTED:
			...
			if dialogue_renderer.dialogue_manager:
				...
				dialogue_renderer.dialogue_manager.advance_to("s07_step6_supported")
		ReasoningWallUI.VerifyResult.INSUFFICIENT:
			...
			if dialogue_renderer.dialogue_manager:
				...
				dialogue_renderer.dialogue_manager.advance_to("s07_step6_insufficient")
		ReasoningWallUI.VerifyResult.CONTRADICTORY:
			...
			if dialogue_renderer.dialogue_manager:
				...
				dialogue_renderer.dialogue_manager.advance_to("s07_step6_contradictory")

func _handle_verified() -> void:
	current_phase = GamePhase.PHASE1_COMPLETE
	_show_notification("✅ 场景07勘查验证通过！")
	StarRatingSystem.add_reasoning(1)
	if dialogue_renderer.dialogue_manager:
		dialogue_renderer.dialogue_manager.set_verify_result("VERIFIED")
		dialogue_renderer.dialogue_manager.advance_to("s07_step6_verified")
...
func _on_scene_node_entered(node_id: String) -> void:
	if node_id == "s7_end":
		...
		_show_notification("🎉 场景七完成！凶器确认（南美箭毒药丸），同一凶手坐实。")
