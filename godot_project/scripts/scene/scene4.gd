...
## 人证调查类型的「六步闭环」：夜访兰斯 → 六方向追问 → 证词提取 → 知识检索
...
func _enter_arrival() -> void:
	...
	# 对齐 08 稿 场景四·入场（L1809-1839）：深夜敲门遇冷 → 亮半镑金币 → 进屋 → 对华生解释"先找兰斯"
	_start_dialogue([
		_mk_node("e0","系统","（演出）深夜，奥德利大院。昏暗的煤气灯下，四十六号房门紧闭。福尔摩斯上前敲门。","guide",["e1"]),
		_mk_node("e1","兰斯警士","（门开一条缝，睡眼惺忪）谁啊？这大半夜的……","click",["e2"]),
		_mk_node("e2","福尔摩斯","（掏出半镑金币，在指间转了转）兰斯警士，耽误你几分钟。关于劳瑞斯顿花园街的案子，想请你再说说当时的情况。","click",["e3"]),
		_mk_node("e3","兰斯警士","（眼睛一亮，语气立刻变了）哦……当然可以，先生。请进，请进。","click",["e4"]),
		_mk_node("e4","福尔摩斯","（转向玩家，低声）他在局里说过一遍了，但正式报告里总有遗漏——你想从哪儿问起？","click",["e5"]),
		_mk_node("e5","华生","（也压低声音）福尔摩斯，我还是不太明白——我们不是应该先去追查那枚戒指吗？为什么来找一个巡警？","click",["e6"]),
		_mk_node("e6","福尔摩斯","（微微一笑）华生，戒指是诱饵，但我们得先知道鱼长什么样。兰斯虽然不起眼，却是第一个到现场的人——他看见的东西，可能比他自己意识到的多得多。","click",["e7"]),
		_mk_node("e7","华生","（恍然大悟）原来如此……我明白了。","click",["e8"]),
...
func _start_step1() -> void:
	...
	_start_dialogue([
		...
		_mk_node("s2","兰斯警士","我赶紧吹警笛，摩契他们俩很快就来了。对了——我出来的时候，门口还靠着个醉汉，唱得震天响，站都站不住。","clue",["s3"],[CLUES["C_SOTCB_401"]]),
...
func _dir_look() -> void:
	...
	_start_dialogue([
		_mk_node("l0","福尔摩斯","那个醉汉长什么样？你看清了吗？","click",["l1"]),
		_mk_node("l1","兰斯警士","（挠头）高个子，肯定比我高。脸是红的——喝多了嘛。别的嘛……胡子？好像有胡子，记不太清。","clue",["l2"],[CLUES["C_SOTCB_402"], CLUES["C_SOTCB_403"]]),
		_mk_node("l2","福尔摩斯","高个、红脸。和花园街现场那串高大足迹、还有……（他没说下去）对上了。华生，记上。","click",["end"]),
	], "l0", _start_panel)
...
func _dir_cloth() -> void:
	...
	_start_dialogue([
		...
		_mk_node("c1","兰斯警士","一件棕色外衣，没错。帽子……好像戴着顶帽子。手里？手里没拿什么吧……","clue",["c2"],[CLUES["C_SOTCB_404"]]),
...
func _enter_transition() -> void:
	...
	_start_dialogue([
		...
		_mk_node("z1","系统","（走出奥德利大院，夜色中）","guide",["z2"]),
