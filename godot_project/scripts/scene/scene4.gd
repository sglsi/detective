...
func _start_step1() -> void:
	...
	_start_dialogue([
		...
		_mk_node("s3","福尔摩斯","（点头，看向玩家）他说的这些，你记下了什么？","click",["s3_e","s3_n","s3_h"]),
	_mk_node("s3_e","福尔摩斯","（低声）四点都关键：凌晨两点、下着雨、醉汉站在门口、还唱着歌。逐一追问，把每条都坐实。","click",["s3_n"],[],"指导",1),
	_mk_node("s3_n","福尔摩斯","四点都关键：时间、天气、醉汉、歌声。用六方向追问把它们挖出来。","click",["s3_h"],[],"指导",2),
	_mk_node("s3_h","福尔摩斯","（看向玩家）……他漏了什么，你自己判断。","click",["s4"],[],"从容",3),
