extends GameScene
class_name Scene06Game

## 场景06：陶尔魁里卡彭蒂耶公寓 —— 自主探索（无引导）
...
func _setup_ui() -> void:
	...
	if scene_title:
		scene_title.set("text", "陶尔魁里卡彭蒂耶公寓")
	_setup_side_panel()
...
func _start_tutorial() -> void:
	scene_controller.load_scene("sc_06_apartment")
	await get_tree().create_timer(0.6).timeout
	dialogue_renderer.load_dialogue_resource("res://resources/dialogues/scene_06_apartment.tres")
...
func _clue_name(id: String) -> String:
	var names = "top_hat": "死者礼帽（帽店溯源）", "photo": "合影照片（体型不符）", "alice": "爱莉丝证词（不在场）", "telegram": "克利夫兰电报（J.H.）"
	...
	match result:
		...
		ReasoningWallUI.VerifyResult.SUPPORTED:
			...
			if dialogue_renderer.dialogue_manager:
				...
				dialogue_renderer.dialogue_manager.advance_to("s06_step6_supported")
		ReasoningWallUI.VerifyResult.INSUFFICIENT:
			...
			if dialogue_renderer.dialogue_manager:
				...
				dialogue_renderer.dialogue_manager.advance_to("s06_step6_insufficient")
		ReasoningWallUI.VerifyResult.CONTRADICTORY:
			...
			if dialogue_renderer.dialogue_manager:
				...
				dialogue_renderer.dialogue_manager.advance_to("s06_step6_contradictory")

func _handle_verified() -> void:
	current_phase = GamePhase.PHASE1_COMPLETE
	_show_notification("✅ 场景06勘查验证通过！")
	StarRatingSystem.add_reasoning(1)
	if dialogue_renderer.dialogue_manager:
		dialogue_renderer.dialogue_manager.set_verify_result("VERIFIED")
		dialogue_renderer.dialogue_manager.advance_to("s06_step6_verified")
...
func _on_scene_node_entered(node_id: String) -> void:
	if node_id == "s6_end":
		...
		_show_notification("🎉 场景六完成！卡彭蒂耶中尉嫌疑排除，调查转向斯特兰森。")
