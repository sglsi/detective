...
func _create_observers() -> void:
	...
	_watson_obs.setup(sa, _ui._dialogue_label, _ui._speaker_label, [
		...
		{"id":"wrist","label":"肤色黑白分明","x":580,"y":400,"w":100,"h":60,"desc":"手腕处肤色分界明显——长期暴露于热带阳光，刚从热带归来",
		 "crop":{"x":0.05,"y":0.155,"cx":0.44,"cy":0.445},"image":"res://assets/characters/watson/watson_teaching.png","anchor":"wrist"},
		{"id":"arm","label":"左臂损伤","x":450,"y":450,"w":100,"h":70,"desc":"左臂动作略显僵硬——战场负伤留下的旧疾",
		 "crop":{"x":0.42,"y":0.16,"cx":0.96,"cy":0.395},"image":"res://assets/characters/watson/watson_teaching.png","anchor":"shoulder"},
		...
		{"id":"medical","label":"身上有消毒液气味","x":500,"y":430,"w":120,"h":120,"desc":"身上有淡淡的消毒液气味——长期接触医院与战地救护的印记",
		 "crop":{"x":0.10,"y":0.0,"cx":0.90,"cy":0.60},"image":"res://assets/characters/watson/watson_teaching.png","anchor":"torso"},
...
func _show_mrs_hudson_dialogue() -> void:
	...
	nodes.append(_dn("h_w","华生","……我更想知道你是怎么看出来的。","click",["hm1"],"思考",0,"小声嘟囔"))
	# —— 侦探学方法论（原著《血字的研究》·福尔摩斯论观察与推理）——
	# 位置：华生追问「怎么看出来的」之后、三难度分流之前，全难度共享（difficulty_filter=0），
	# 让玩家在动手观察前先拿到方法论，学以致用。末节点 hm7 的 next 回到原分流链。
	nodes.append(_dn("hm1","福尔摩斯","研究人类最恰当的途径，还是从具体的人着手。","click",["hm2"],"哲理",0,"放下茶杯"))
	nodes.append(_dn("hm2","福尔摩斯","一个逻辑学家不需要亲眼见到大西洋或尼亚加拉瀑布——他能从一滴水上，推测出它们有可能存在。","click",["hm3"],"哲理",0,"把茶杯举到光下"))
	nodes.append(_dn("hm3","福尔摩斯","整个生活就是一根巨大的链条，只要见到其中一环，整根链条的模样就可以推想出来。","click",["hm4"],"哲理",0,"用雪茄在空中轻轻一划"))
	nodes.append(_dn("hm4","福尔摩斯","推断和分析的科学，也像其他技艺一样，只有经过长期而耐心的钻研才能掌握。人们尽毕生精力，也未必能达到登峰造极的地步。","click",["hm5"],"哲理"))
	nodes.append(_dn("hm5","福尔摩斯","所以初学的人，不妨先从较浅显的问题入手：比如遇到一个人，要在瞬息之间辨识出他的经历和职业。","click",["hm6"],"指导"))
	nodes.append(_dn("hm6","福尔摩斯","这种锻炼看似幼稚无聊，却能让你的观察力变得敏锐起来，教你应该从哪里看、应该看些什么——手指甲、衣袖、靴子、裤子的膝盖部分、大拇指与食指之间的茧子、表情、衬衣袖口，随便哪一处，都能清清楚楚地显露出他的职业。","click",["hm7"],"指导",0,"目光扫过华生的袖口与手背"))
	nodes.append(_dn("hm7","福尔摩斯","现在，就从这位军医身上开始吧。","click",["h5_e","h5_n","h5_h"],"从容",0,"把雪茄放回唇边，向华生抬了抬下巴"))
	# 不同难度不同引导：h5 之后分流，难度过滤节点须「链式为 next」(h5_e→h5_n→h5_h→end)，
	# 引擎 should_show 跳过隐藏变体时才会依次走到第一个可见变体（否则会误判 end 提前结束）。
	nodes.append(_dn("h5_e","福尔摩斯","职业就藏在这些地方。从手腕晒痕、左臂旧伤、脸色黝黑、面容憔悴、军人站姿、身上的消毒液气味下手，每一条都要讲得出证据来。","click",["h5_n"],"指导",1,"低声"))
	nodes.append(_dn("h5_n","福尔摩斯","别急着下结论。先把看到的每一处细节记牢，再问自己：它们能串成什么？","click",["h5_h"],"指导",2,"点头"))
...
func _show_opening_dialogue() -> void:
	...
	# —— 普通（NORMAL）：标准提示 ——
	nodes.append(_dn("s0_n","福尔摩斯","证据都在他身上——手指甲、衣袖、靴子、膝盖、手上的茧，任何一处都别放过。手腕、左臂、脸色、面容、站姿，还有那股消毒液的气味，先把它们找齐。","click",["s1_n"],"从容"))
...
func _show_watson_verdict_dialogue(v: int) -> void:
	...
	if v >= 3:
		...
		nodes.append(_dn("wv3","福尔摩斯","如果把这些情形联系起来，还不能使案件的调查人恍然领悟，那几乎是难以想象的。","click",["wv4"],"哲理"))
		nodes.append(_dn("wv4","福尔摩斯","记住：对一个伟大人物来说，任何事情都不是微不足道的。","click",["wv5"],"哲理"))
		nodes.append(_dn("wv5","福尔摩斯","不错。你已经摸到门道了。","click",["wv6"],"认可",0,"点了一下头"))
		nodes.append(_dn("wv6","福尔摩斯","犯罪行为都有非常类似的地方。如果你对一千件案子的详情细节都能了如指掌，却对第一千零一件无从解释，那才是怪事呢。","click",["wv7"],"哲理",0,"望向窗外"))
		nodes.append(_dn("wv7","华生","福尔摩斯认可了——这可不容易。","click",["end"],"思考",0,"在小本子上记"))
	else:
		nodes.append(_dn("wv1","福尔摩斯","方向是对的，但还有关键的细节被你漏掉了。别急——这门技艺，只有长期而耐心的钻研才能掌握。","click",["wv2"],"从容",0,"没评价，只递过放大镜"))
