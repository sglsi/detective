...
func _show_detective_dialogue() -> void:
	...
	# ⚠️ 不同难度不同引导：j2 之后分流（j2_e→j2_n→j2_h→end），难度过滤节点链式为 next。
	_start_dialogue([
		_mk_node("j0","福尔摩斯","（目光扫过房间）先别急着下结论，华生。斯特兰森是刀伤，德雷伯是服毒——手法不同，但别的或许一样。把屋子每样东西都看一遍。","click",["j1"]),
		_mk_node("j1","雷斯垂德","窗户虚掩着，凶手多半从那儿溜的。和头一桩空屋案一个路数。可死法不一样啊……","click",["j2"]),
		_mk_node("j2","福尔摩斯","同一只手，可以换刀子。华生，戴上放大镜——把屋子看一遍。","click",["j2_e","j2_n","j2_h"]),
	_mk_node("j2_e","福尔摩斯","（低声）重点：尸体、窗户、那两粒药丸、还有电报——四处都别漏，用放大镜和试剂盒逐一查。","click",["j2_n"],[],"指导",1),
	_mk_node("j2_n","福尔摩斯","尸体、窗户、药丸、电报——四处都查到，别只看最显眼的。","click",["j2_h"],[],"指导",2),
		_mk_node("j2_h","福尔摩斯","（环视）……屋里每处都可能是线索。你自己看。","click",["end"],[],"从容",3)], "j0", _on_detective_ended)
