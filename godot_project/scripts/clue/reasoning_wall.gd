...
var _verify_confirm_backdrop: ColorRect = null  # 确认框遮罩——须与确认框同生命周期清理，否则残留全屏 STOP 层导致界面假死
var _verify_result_backdrop: ColorRect = null   # 结果窗口遮罩——须与结果窗口同生命周期清理
...
func _create_ui() -> void:
	...
	_left_panel.offset_right = 346  # 432 的 4/5 ≈ 346（用户二次缩窄）
...
func _on_open_graph_view() -> void:
	...
	gv.z_index = 5
	# 契入让出区须与顶栏底(offset_bottom=110)/左栏右缘(offset_right=346)一致；
