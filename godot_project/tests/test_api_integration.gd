...
func _run_all_tests() -> void:
	...
	await _test("APIConfig.get_base_url() 返回有效地址", func():
		...
		assert_ok(url.begins_with("http"), "URL 应以 http 开头")
		assert_ok(url.ends_with("3000") or url.contains("api."), "URL 应指向后端")
	)
	
	await _test("APIConfig 端点定义完整", func():
		assert_ok(APIConfig.ENDPOINTS.has("health"), "缺少 health 端点")
		assert_ok(APIConfig.ENDPOINTS.has("login"), "缺少 login 端点")
		assert_ok(APIConfig.ENDPOINTS.has("saves_upload"), "缺少 saves_upload 端点")
		assert_ok(APIConfig.ENDPOINTS.has("progress_get"), "缺少 progress_get 端点")
	)
	
	# ---- 2. APIManager 初始化 ----
	await _test("APIManager 已加载", func():
		assert_ok(APIManager != null, "APIManager 应为非空")
		assert_ok(APIManager.base_url != "", "base_url 不应为空")
	)
	
	await _test("APIManager 默认状态正确", func():
		var status = APIManager.get_status()
		assert_ok(status.has("online"), "状态应包含 online")
		assert_ok(status.has("pending_count"), "状态应包含 pending_count")
		assert_ok(status["pending_count"] >= 0, "pending_count 应 >= 0")
	)
	...
	await _test("GET /api/health 成功", func():
		...
		assert_ok(data.get("status", "") == "ok", "status 应为 ok")
	)
	...
	await _test("POST /api/auth/guest 创建会话", func():
		...
		if result.get("error", true):
			...
			return
		var data = result.get("data", {})
		assert_ok(data.has("guest_id"), "应返回 guest_id")
		assert_ok(data.has("expires_at"), "应返回 expires_at")
		assert_ok(APIManager.guest_id != "", "guest_id 应已保存")
	)
	
	# ---- 5. AuthManager ----
	await _test("AuthManager 初始为游客状态", func():
		assert_ok(AuthManager.current_auth_state == AuthManager.AuthState.GUEST, "初始应为 GUEST")
		assert_ok(AuthManager.is_guest(), "is_guest 应为 true")
	)
	
	await _test("AuthManager.get_username() 游客模式返回默认名", func():
		var name = AuthManager.get_username()
		assert_ok(name != "", "用户名不应为空")
	)
	...
	await _test("SaveManager 已加载", func():
		assert_ok(SaveManager != null, "SaveManager 应为非空")
		assert_ok(SaveManager.save_version == 1, "save_version 应为 1")
	)
	...
	await _test("离线请求加入队列", func():
		...
		assert_ok(APIManager.get_pending_count() == initial_count + 1, "队列应增加1")
	)
	
	# ---- 9. 信号连接 ----
	await _test("SystemEventBus 信号已定义", func():
		assert_ok(SystemEventBus.has_signal("user_registered"), "缺少 user_registered 信号")
		assert_ok(SystemEventBus.has_signal("user_logged_in"), "缺少 user_logged_in 信号")
		assert_ok(SystemEventBus.has_signal("game_saved"), "缺少 game_saved 信号")
		assert_ok(SystemEventBus.has_signal("game_loaded"), "缺少 game_loaded 信号")
		assert_ok(SystemEventBus.has_signal("network_online"), "缺少 network_online 信号")
		assert_ok(SystemEventBus.has_signal("network_offline"), "缺少 network_offline 信号")
	)
	
	# ---- 10. 错误处理 ----
	await _test("AuthManager.logout() 清除状态", func():
		AuthManager.logout()
		assert_ok(AuthManager.is_guest(), "退出后应为游客")
		assert_ok(APIManager.auth_token == "", "token 应已清除")
	)
