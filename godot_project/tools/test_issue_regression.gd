...
func _initialize() -> void:
	...
	if BE != null:
		...
		for s in missing:
			if "C-MAIN" in s and "NPC_WT" in s:
				watson_missing = true
		_chk(not watson_missing, "Issue3 华生墙 C-MAIN→NPC_WT(target 金边) 命中真相、不报未连接  (missing=%s)" % str(missing))
		# 反向校验：玩家用「完整链路」但把结论→人物的金边误用 support（非 target），
		# 该 branch 仍激活（前 4 条边命中），但 target 金边应判缺失（kind 精确比对生效）。
		var wrong_rels := [
			{"from": "wrist", "to": "W-A1", "kind": "support", "dashed": false},
			{"from": "W-A1", "to": "C-A1", "kind": "support", "dashed": false},
			{"from": "C-A1", "to": "C-A2", "kind": "support", "dashed": false},
			{"from": "C-A2", "to": "C-MAIN", "kind": "support", "dashed": false},
			{"from": "C-MAIN", "to": "person:NPC_WT", "kind": "support", "dashed": false},
		]
	...
	if GV != null:
		...
		# 关系方向必须 from=子(to)，to=父(被推导)：C2→C3、C1→C2，使 C3 为祖先，折叠可收下游。
		gv._relations = [
			{"from": "conclusion_C2", "to": "conclusion_C3", "kind": "support"},
			{"from": "conclusion_C1", "to": "conclusion_C2", "kind": "support"},
		]
		...
		_chk("conclusion_C2" in desc_c3 and "conclusion_C1" in desc_c3,
			"Issue1 结论链 descendants(C3) 含下游 C2/C1  (desc=%s)" % str(desc_c3))
		gv._folded_nodes = {"conclusion_C3": true}
		var hidden: Dictionary = gv._fold._compute_hidden()
		_chk(hidden.has("conclusion_C2") and hidden.has("conclusion_C1"),
			"Issue1 折叠 C3 后 hidden 含下游 C2/C1  (hidden=%s)" % str(hidden.keys()))

		# Issue4：_relayout_on_edge 标志在 _compute_layout 内被消费归位
		# 需先给画布有效尺寸（否则 _compute_layout 在 owner._canvas.size 处崩，复位逻辑不执行）
		if gv._canvas == null:
		...
		# Issue4：_add_edge 点亮标志（连边即触发重排请求）；_add_edge 是 GraphViewEdge 的方法
		gv._layout._relayout_on_edge = false
		gv._state = GraphViewController.State.EDITABLE
		gv._edge._add_edge("conclusion_C1", "conclusion_C2", "support")
