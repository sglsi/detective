...
- **XMind 式自由布局**：`_compute_layout` 的 MODE_C 改为调用 `_xmind_layout`（原本的同心圆 ring 逻辑移除）。中心焦点人物为根（严格居中）；**结论固定正上方**，其余推断/结论/推理链主分支从正上方之外的整圈均匀扇出（同半径同层，`ang_of` 按 `(slot+1)/n*TAU` 分配避免与结论重叠）；线索作为子主题挂在其佐证主分支（`relation_tags`/`related_to` 命中主分支）**外侧向外延伸并轻微交替横摆**（`r_main+160+k*92` ±58），无佐证线索作自由主题在最外层散布。玩家拖动过的位置沿用；布局收尾对全部节点 `_clamp_to_canvas`（margin 60），防止默认布局把文本框挤出可视区。拖拽落下同样走 `_clamp_to_canvas(new_center)`，不再按 kind 钳回 band。`_xmind_layout` 用 `else` 包裹避免裸 `return`（项目把 void 函数返回当错误）。可复用 `tools/test_xmind_diag.gd` 校验该结构。
