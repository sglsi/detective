...
- **XMind 式自由布局**：`_compute_layout` 的 MODE_C 改为调用 `_xmind_layout`（原本的同心圆 ring 逻辑移除）。中心焦点人物为根；推断/结论/推理链作为主分支，从正上方开始均匀扇出（`ang_of`）；线索沿其佐证分支（`relation_tags`/`related_to` 命中主分支）向外剖列并轻微横向扇出；无佐证线索在最外层柔性散布。玩家拖动过的位置直接沿用，**不再按 kind 钳回 band**——拖拽落下改用 `_clamp_to_canvas(new_center)`（仅限画布内，margin 60），`_clamp_to_band` 与 `_node_width_for_kind` 保留但已不再被调用。`_xmind_layout` 已去掉裸 `return`（项目把 void 函数返回当错误），空主分支走 `else` 分支用极坐标散布兜底。
