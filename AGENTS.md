...
- **洞察星改源（方案已议定方向，待实施）**：用户决定「推理战场」不再作为洞察星数据源，改由图谱「推断→结论连线及两边节点」评价（合成层完整度）；方案 B（战场联动引导）/C（放宽阈值 80/55/25）已明确舍弃。
- **存档导出与复盘工具（诊断链路）**：侧栏 side_panel 新增「📤 导出」按钮——读 `SaveManager.get_slot_list_sorted()[0]` 最新槽位，白名单导出（slot/timestamp/scene_id/difficulty/star_chains/collected_clues/case_wall_state/scene_state）JSON；Web 下 `Marshalls.utf8_to_base64` + data URI 自动下载 + TextEdit 弹窗可手动复制。拿到 JSON 后跑 `GODOT --headless --path . -s res://tools/analyze_save_report.gd -- <save_export.json>` 复盘：复用 WallBranchEvaluator（同一评分引擎）复算两堵教学墙（scene_state 里 `wall_state_watson`/`wall_state_messenger`），打印 verdict/ratio/逐链明细/战场按钮状态。**注意：`_persist_state` 只持久化 associated/milestones_lit/battlefield/verified/verdict/doubt_book/relations——graph_nodes/derived_conclusions 不入档**，复盘工具按「relations 端点出现即视为节点产出」近似。云存档链路不可用（develop 库无 game_saves 表，迁移未跑；Web 构建存档实际只在浏览器 IndexedDB）。
