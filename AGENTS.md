...
- **对话头像通用构图（2026-09-04）**：NPC bust 通用构图 = 512x512 画布、内容占 **90~98% 高、底对齐、顶部 y≈8-50 起笔**（实测 gregson 96%/lestrade 90%/alice 90% 等）。此前 hudson_bust 内容只占 53% 高偏下（顶部 26% 裁片 contain 后放大不足），用户看到「头像小且缩在对话框下半」。重裁：源图（1024x1024）取 x340-674/y20-470（头+肩+上胸+交叠手），放大到内容高 89% 底对齐。**教训：改头像前先测同目录其他 bust 的 alpha bbox 占比对齐通用构图，不是「裁出多少就显示多少」**。
