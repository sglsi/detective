...
- **mislead_chance 初始化矛盾（2026-09-08 已修，用户拍板方案②+读档兜底）**：`difficulty_manager.gd` `_ready()` 原 pass 不自举——已改为 `set_difficulty(current_difficulty)`：未走难度选择的玩家按完整 NORMAL 配置跑（chance=0.3、hotspot_hint_level=1 淡圈、dynamic_hint_chance=true 等 7 参数从声明默认落地为 NORMAL 分支值，其中 5 个参数当时尚无消费方）。**读档兜底**：`save_manager.gd` `data.get("difficulty", DifficultyManager.current_difficulty)`——存档缺 difficulty 字段时延续当前难度，不再默认 0=EASY（存档里显式 0 仍是玩家选的 EASY，语义保留）。影响：信使墙 M-05/06 干扰假设进墙（"信号 vs 噪音"教学恢复）、场景 2-8 观察面板干扰热点开始按确定性概率出现、推理墙结论候选开始掺误导项——**此前从未出现过，玩家会感知新热点/新提示**。
- **Godot 工具链播种脚本重建（2026-09-08，沙箱回收后重播种）**：`tools/godot/setup_godot.sh` 已入库（`.gitignore` 从忽略整个 `/tools/godot/` 改为只忽略 `*.zip`/`*.tpz`/`editor_extract/`——脚本不再随回收丢失）。**三坑**：①GitHub 资产名带小写 v（`Godot_v4.7.1-stable_linux.x86_64.zip`）——文件名漏 v 一律 404，且"Not Found"9 字节会被误判为限流/大小阈值（此前臆测全错，用真实资产名一击即中）；②gh.ddlc.top 现行格式**不带** `https://` 前缀（`https://gh.ddlc.top/github.com/...`，带前缀 404）；③分片必须串行+片间 sleep（实测 16MB 分片稳定，32MB+ 或并发 8 会 404——是否真限流未证，串行已够用）。
- **背景图 4K 重绘（2026-09-14，用户"分辨率不够"反馈）**：场景一两张背景图生图重绘为 4K——`screen01-sofa01`（1920×1440→4096×3065）与 `screen01-opendoor`（1024×1024→4096×4096）。流程：原图上传对象存储拿签名 URL → SDK `generate(image=url, size="4K")` 图生图（prompt 只描述"保持构图/色调/光影一致、仅提升细节"，模型看得见原图无需人工描述画面）→ 多模态（qwen-3-5-plus）左右对比验证 PASS → 缩到 ≤4096 宽（**sofa 原始生成 4704 超上限**）→ 存 JPG q88（PNG 14MB 太大，JPG 2.4MB/2.1MB）→ scene1.gd 三处 load 路径 .png→.jpg → pck +22MB（4K ctex 物理代价，131MB）。**坑**：①`--check-only --script` 与 `-s _init` 同步 load 场景脚本会报 "Identifier not found: SaveSystem"（全局 class 缓存未就绪）——探针必须 `call_deferred("_run")` 延迟到主循环启动（t_letter_view_smoke 模式），否则误判编译失败；②coze-coding-ai CLI 无 upload/image-to-image 子命令，必须走 Python SDK。
- **沙发背景阳光版（2026-09-14，用户"加阳光光束增加动态感"反馈）**：sofa01 二次图生图（输入=4K 无光版），prompt 描述"单一柔和金色晨光光束从窗户射入+光尘+窗框光斑投地板/家具，强调 subtle/delicate/NOT overexposed"。多模态验证 PASS（构图一致/光束从左窗射入/地板窗框光斑/沙发书架明暗交错）。落地同流程，戳 v=20260914-bg2，pck 130.5MB。**LLM SDK 多模态调用法**：`LLMClient.invoke(model=..., messages=[HumanMessage(content=[{'type':'text',...},{'type':'image_url','image_url':{'url':'data:image/jpeg;base64,...'}}])])`——messages 必须是 langchain HumanMessage 对象（传 dict 报 "must contain at least one HumanMessage"），client 方法是 invoke/stream（无 chat）。
- **版本**：v=20260906v。
...
- **人物裁剪禁止含脸（2026-09-07 用户裁定，长期有效）**：任何线索部位放大裁剪/特写框**不得包含人物脸部或被截断的脸部边缘**（哪怕是 1/4 张脸、下巴一角）——对人物不尊重，会引起玩家不适。标定锚点时 y 上缘必须验证避开脸区（watson_teaching.png 脸区 y 0~0.14、x 0.35~0.62），每次锚点调整后必须用多模态复核"是否含脸"。此规则适用于今后所有角色图/裁剪/头像/立绘相关的切图工作
...
- **图谱契入让出（2026-08 现用方案，替代契回全屏+命中分离）**：图谱契回墙顶层铺满（`_on_open_graph_view` 里 `add_child(gv)` + `PRESET_FULL_RECT` + `gv.z_index=5`），但图内部 `_clip` 契入让出「左栏右侧、顶栏之下」的图谱交互区（`_clip.offset_left=hit_off_left / .offset_top=hit_off_top`，`clip_contents=true`）。`_clip`/`_canvas` 均 `MOUSE_FILTER_STOP` + `gui_input.connect(_on_canvas_gui)`——**同一区域既显示又承接全部画布交互（平移/滚轮/空白点击/shift 建边/折叠）**，顶栏/左栏不被图谱覆盖故天然优先可点，无需命中分离层（曾用 `_hit_layer` 分离让 `_on_canvas_gui` 脱链导致 shift 连线/折叠失效，已废除）。拓扑根 `self.mouse_filter=IGNORE`。
  ...
  - **让出区单源传入**：图谱用成员 `hit_off_top/hit_off_left`（默认 110/540，**当前设 110/346——2026-09-07 左栏两轮缩窄（540→432→346）后同步**），由 `_on_open_graph_view` 处设 `gv.hit_off_top=110`、`gv.hit_off_left=432`（对齐顶栏底 `_top_bar.offset_bottom` / 左栏右缘 `_left_panel.offset_right`）。改 UI 尺寸须同步这两值（`tools/q6_contract_guard.gd` 源码契约守卫 Q6 会断言：clip/canvas 均 STOP+gui_input、无 `_hit_layer`、契入偏移用 `hit_off_*`、`_zoom_at`/`fit_view` 含 `_clip.get_global_transform().origin`）。
...
- **放大镜取景错位修复（2026-09-06，用户实证「镜内内容与镜位置不一致」，此前 a9f86fa 已修过一次视觉中心）**：根因=`tool_bar.gd _process` 里 `lens_center = mp / vp.size` 是**游戏区 UV**，而着色器 `SCREEN_UV` 是**全窗口 UV**（含 `stretch mode=canvas_items` 默认 aspect=keep 的黑边）——窗口非 16:9 时两者错位黑边的一半（之前修复的是 top_level 视觉中心一致性，窗口恰 16:9 时两种 UV 相同故未暴露本根因）。修复：`lens_center = (vp.get_final_transform() * mp) / DisplayServer.window_get_size()`（final_transform 把鼠标点映射到窗口像素，与 SCREEN_UV 同空间；窗口恰 16:9 时数学退化为旧算法不破坏行为）。**教训：SCREEN_UV 永远以窗口为参照系，canvas_items 拉伸下鼠标/节点坐标是游戏区参照系，二者换算必须过 get_final_transform**。
- **ESC 关闭修复**：关闭处理用 `call_deferred("_on_close_pressed")`，避免在 `_input()` 里销毁节点卡死。
...
- **华生教学链过渡结论去除（2026-09-10，用户要求）**：删除 CH01W 过渡结论「承受了这个年龄本不该承受的伤痛」(原 C-C1)，并将「左臂受过伤未完全恢复」(W-C1) 与「久病初愈而又历尽了苦难」(W-C2) 两条推断的**结论直接指向**「不该有的伤害只可能来自军事任务」(C-C2)——即把 C-C2 的 `gate_hypo_ids` 由 `["conclusion_C-C1"]` 改为 `["W-C1","W-C2"]`，伤痛线由 线索→推断→结论→结论（3 层）缩短为 线索→推断→结论（2 层）。同步：①`reasoning_chains.gd` 唯一事实源删除 C-C1 条目、改 C-C2 gate、里程碑 MW-3 改为「旧伤与久病指向军事任务」；②`case_branch_truth.gd` 的 CH01W legacy 注释/节点/边同步（注：CH01W 已迁入 ReasoningChains，`branches()` 走 `derive_truth()` 机械派生，legacy 条目为死代码，仅注释参考）；③技术文档 `data/reasoning_chains_all_scenes.md` 华生墙段重写（原文档已过时，仍引用不存在的 W-C3 / "华生参加过战争"）。维护规则不变：真相链必须与 battlefield 的 gate 同源，改链只改 `reasoning_chains.gd`。
...
### 华生线索裁剪重标（2026-09-06，用户五处反馈）+ 换图出问题的原因分析
- **根因（上次换 watson03 后裁剪出大量问题的原因）：归一化坐标跨图沿用**。换图时用户说"之前左肩锚定范围很好"，我直接把 044519a（512×512 半身构图旧图时代）的 shoulder=cx0.594 原数搬到 640×1663 全身构图新图——**归一化数值相同≠画面位置相同**：0.594 在旧图落左肩、在新图落点偏左成了左胸。torso"上半身"给了 0.32×0.32 小框（胸局部），与"腹部以上全部区域"语义差很远；wrist 只框了手掌，用户语义是"手掌+手臂"整段。**教训：换图后所有锚点/裁剪必须按新图 PIL 实测重新标定；用户认可的"之前的范围"指的是视觉位置而非数值；部位语义（"上半身""手掌+手臂"）按解剖范围给全，遵循"主要线索完整"原则宁大勿小**。
- 本次新值（watson03 实测）：wrist=手掌+前臂完整（cx0.22/cy0.30/w0.36/h0.36，x0.04-0.40 y0.12-0.48）、shoulder=左肩+左上臂为主（人物左=画面右侧，cx0.74/cy0.26/w0.28/h0.32）、torso=腹部以上全区（cx0.50/cy0.30/w0.80/h0.60，y0-0.60 全身宽，含头/手是设计使然）、pose 军人气质=视图全身、锚点按钮置胸部（cy0.38）。face 不变。scene1.gd 三处 crop 同步（face/pose 不变）。test_watson_v6_anchor 断言按新语义重写（torso 改查尺寸，删"不与 face/wrist 重叠"检查；新增 wrist-shoulder 左右不重叠）CLUE_ANCHOR_OK。
- **wrist 第三轮修正（2026-09-07，用户反馈"手掌依旧显示不全"）**：第二轮值（右缘 0.40）仍切掉指尖——**PIL 肤色连通域+alpha 轮廓都测不到指尖真实右缘**（指尖有深棕描边+阴影，肤色规则漏检；指尖垂在身前与躯干 alpha 混叠，行轮廓右缘失真为 0.348）。最终用**多模态视觉模型（llm skill doubao-seed-2-0-pro）看裁切图**定位：指尖右缘 0.427、下缘约 0.45，且图内有两只手需先分辨（"肤色黑白分明"=画面左侧卷袖口的右手，画面右侧另有左手）。终值 cx0.28/cy0.30/w0.36/h0.44（x 0.10-0.46 y 0.08-0.52），多模态复核三达标：无边缘切断/卷袖口可见/手为主体。**拇指被外套遮挡是原图构图，裁切无法解决**（复核时区分"衣物遮挡"与"边缘切断"）。**教训：①肤色/alpha 规则测带描边边缘的部位会系统性偏窄，手指指尖这类精细部位直接上多模态模型看图 bbox 最可靠；②同一图内多处疑似部位（两只手）先用多模态分辨哪个是线索目标再定锚；③多模态输出有噪声，用裁切图复核收敛 2-3 轮。**测试防回归：断言 wrist 右缘≥0.44（指尖右缘 0.427+余量）。v=20260906j（pck 71920732）。
- **wrist/shoulder 第四轮标定（2026-09-07，用户上传两张参考图）**：①wrist 上缘 0.08 裁进了 1/4 张脸（脸区 y0-0.14 与 x0.35-0.46 重叠）——用户明确"对人物不尊重"，已入长期约束（见用户偏好·人物裁剪禁止含脸）；②用户提供期望构图参考图（手部特写、肩部领口特写），**多模态"参考图→全图定位 bbox"不可靠**（比例对不上：参考 0.84 vs 模型定位 0.43），改为"按参考图宽高比+解剖特征反推+裁图回验"：终值 wrist cx0.245/cy0.30/w0.39/h0.29（x 0.05-0.44、y 0.155-0.445，与参考构图一致且完全无脸）、shoulder cx0.69/cy0.278/w0.54/h0.235（x 0.42-0.96、y 0.16-0.395，含肩+领口+上臂、无脸；y 上缘从 0.145 调 0.16 消除左上角下巴边缘）。锚点终值以本段为准。
- **观察器热点点击错位修复（2026-09-07，用户实证"点哪个圆圈都显示同一条线索"）**：根因=`clue_observer._anchor_local_rect` 命中区用 `r = max(rx, ry) * 1.12` 方形半径——竖长图（640×1663）上大锚点（torso w0.8/h0.6、pose w1/h1 全图）产生全屏级按钮互相叠盖，**点击命中由 Button z 序（添加顺序）决定而非位置**：torso(medical/消毒液) 最后添加在最顶层→永远先被点中，记录后隐藏→轮到 pose(军人气质)……完美复现用户描述。
...
### 放大镜滚轮穿透修复 + 左栏二次缩窄（2026-09-07，用户反馈"整个背景都在放大缩小"）
- **滚轮穿透根因链**：放大镜激活时 tool_bar._input 已 `set_input_as_handled`，但 scene_framework 观察相机的滚轮缩放（`_unhandled_input` → `_zoom_at` 缩放 `_world` 背景）分支**没检查 `event.pressed`**——滚轮释放沿（Godot 会补 pressed=false 事件）绕过 pressed-only 拦截直达 `_unhandled_input` 缩背景；且用户此前可能仍在旧版（滚轮=背景缩放是观察相机原有行为）。
- **双层防御修复**：① tool_bar._input 滚轮分支改为 **wheel up/down 不要求 pressed 一律 `set_input_as_handled`**（pressed 变倍率、release 纯拦截）；② scene_framework._unhandled_input 滚轮分支加 `if not event.pressed: return`（释放沿不缩背景，语义修正）。放大镜 hint 文案加"滚轮缩放"提示。
- **左栏二次缩窄**：432 → **346**（432 的 4/5 ≈ 346），`hit_off_left` 同步 346（单源规范）。
- 版本戳 v=20260906o（pck 71919936，PCK_PROBE=NEW，302 带戳验证 ✓）。
...
- **场景一信息揭示时序约束（2026-09-04，用户裁定）**：福尔摩斯的台词**不得提前说出玩家尚未获得的信息**——委托信打开前只谈"有封委托信"，案件内容（地点/男尸/死因）必须来自信件原文（cl1 全文展示）或之后的现场勘查；**"像是中毒"这类无依据过早结论禁止出现**（信原文只说"未发现任何能说明致死原因之证据"）。落地：`scene1.gd _show_commission_letter_dialogue` 的 cl0 改为一句"信使留下的，是葛莱森警长的委托信。"；cl5 删"墙上有血字"（血字是场景二现场核心线索，提前说会削弱发现感）；委托信线索描述同步删"疑似中毒"。设计文档（`godot_project/docs/02_核心设计/02_血字的研究_场景设计与流程.md`）委托信 Step5 段已补"信息揭示时序约束"（原根级副本 `assets/02_…` 已于 2026-09-23 归档至 `archive/2026-09-23_过期设计文档副本/`，规则已回填权威文档）。**维护规则：新增台词先过"信息来源检查"——这条信息玩家此刻通过什么途径获得？**
