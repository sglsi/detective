...
- **存档导出与复盘工具（诊断链路）**：**入口=推理墙顶栏 row2「🗄 存档」按钮**（`reasoning_wall._on_export_save_pressed`）。教训：首版挂在 `side_panel.gd`——该类是**从未实例化的死类**（全项目无引用），按钮永远不可见；加 UI 前必须先确认挂载点真实在 UI 树里。实现：读 `SaveManager.get_slot_list_sorted()[0]` 最新槽位，白名单导出（slot/timestamp/scene_id/difficulty/star_chains/collected_clues/case_wall_state/scene_state）JSON，弹 graph `_show_export_panel` 文本窗复制 + Web 下 `Marshalls.utf8_to_base64` data URI 自动下载。拿到 JSON 后跑 `GODOT --headless --path . -s res://tools/analyze_save_report.gd -- <save_export.json>` 复盘：复用 WallBranchEvaluator（同一评分引擎）复算两堵教学墙（scene_state 里 `wall_state_watson`/`wall_state_messenger`），打印 verdict/branch_ratio/insight_ratio/逐链明细/战场按钮状态。**注意：`_persist_state` 只持久化 associated/milestones_lit/battlefield/verified/verdict/doubt_book/relations——graph_nodes/derived_conclusions 不入档**，复盘工具按「relations 端点出现即视为节点产出」近似。云存档链路不可用（develop 库无 game_saves 表，迁移未跑；Web 构建存档实际只在浏览器 IndexedDB）。
