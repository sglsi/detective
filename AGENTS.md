...
- **观察器热点点击错位修复（2026-09-07，用户实证"点哪个圆圈都显示同一条线索"）**：根因=`clue_observer._anchor_local_rect` 命中区用 `r = max(rx, ry) * 1.12` 方形半径——竖长图（640×1663）上大锚点（torso w0.8/h0.6、pose w1/h1 全图）产生全屏级按钮互相叠盖，**点击命中由 Button z 序（添加顺序）决定而非位置**：torso(medical/消毒液) 最后添加在最顶层→永远先被点中，记录后隐藏→轮到 pose(军人气质)……完美复现用户描述。修复：6 个热点按钮改 `mouse_filter=IGNORE`（保留对象做状态），立绘上加透明点击层 `_hit_layer`（PRESET_FULL_RECT+MOUSE_FILTER_STOP+gui_input），点击时 `_pick_nearest_hotspot` 按**距未记录线索的锚点中心欧氏最近**判定触发——点哪个圆圈（圆圈中心=锚点中心）就命中哪条线索，与 z 序无关。**教训：重叠命中区不能用控件网格命中（z 序决定一切），"多目标就近判定"要么单一覆盖层+最近距离算法，要么命中区互斥收缩。**
